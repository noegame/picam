...
def detect_aruco_tags_advanced(
    ...
    if ids is not None:
        for i, marker_id in enumerate(ids):
            ...
            if homography_inv is not None:
                ...
                point_real = cv2.perspectiveTransform(point_img, homography_inv)
    ...
    if verbose and len(all_detections) > 0:
        if homography_inv is not None:
            print(
                f"\n{'#':<6}{'ID':<10}{'Position Image':<25}{'Position Réelle (mm)':<30}"
            )
            print("-" * 71)
        else:
        ...
        for idx, detection in enumerate(all_detections, 1):
            ...
            if homography_inv is not None and detection["real_coords"] is not None:
                ...
                print(
                    f"{idx:<6}{marker_id:<10}({center_x}, {center_y}){'':>9}({real_x}, {real_y})"
                )
            else:
    ...
    if len(all_detections) > 0:
        ...
        for detection in all_detections:
            ...
            # Ajouter les coordonnées image
            img_text = f"({center_x}, {center_y}) px"
            img_pos = (center_x + 50, center_y + 20)
            cv2.putText(
                output_image,
                img_text,
                img_pos,
