...
class FakeCamera:
    ...
    def __init__(self, w: int, h: int, image_folder: Path):
        ...
        logger.info(f"Initialisation de la fausse caméra depuis le dossier: {image_folder}")
        self.width = w
        self.height = h
        self.image_folder = image_folder
        self.image_files = sorted([p for p in self.image_folder.glob('*') if p.suffix.lower() in ['.jpg', '.png', '.jpeg']])
        
        if not self.image_files:
            raise FileNotFoundError(f"Aucune image trouvée dans le dossier: {self.image_folder}")
    ...
    def capture_image(self, pictures_dir: Path) -> tuple[np.ndarray, Path]:
        ...
        try:
            ...
            logger.info(f"Image 'capturée': {filepath.name} (source: {source_path.name})")
